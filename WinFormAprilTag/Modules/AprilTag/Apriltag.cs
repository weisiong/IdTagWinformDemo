...
namespace csharp
{
    public class Apriltag {
        ...
        Point _interpolate (Point[] p, PointF relativePoint) {
            ...
            double tmp29 = tmp21 * p[2].Y - tmp23 - tmp24 + tmp22 * p[3].Y - tmp25 * p[3].Y + tmp27 + tmp28 - tmp5 * p[2].Y;
            double tmp30 = p[0].X * p[2].Y;
            double tmp31 = tmp23 - tmp25 * p[1].Y - tmp24 + tmp26 * p[1].Y + tmp30 * p[3].Y - tmp27 - tmp0 * p[3].Y + tmp28;
            ...
            return new Point ((int)(hy / hw),(int)(hx / hw));
        }
        /// <summary>
        ...
        public ArrayList detect (Mat frame) {
            ...
            Mat dst = new Mat ();
            Mat gauss = new Mat ();
            CvInvoke.GaussianBlur(gray, gauss, new Size(3, 3), this.sigma);
            switch (this.threshold) {
                case "canny":
                    CvInvoke.Canny(gauss, dst, 150, 400, 3);
                    break;
                case "adaptive":
                    CvInvoke.AdaptiveThreshold(gauss, dst, 255, AdaptiveThresholdType.GaussianC, ThresholdType.BinaryInv, 9, 5);
                    break;
                default:
                    CvInvoke.Canny(gauss, dst, 150, 400, 3);
                    break;
            ...
            CvInvoke.FindContours(dst, contours, hierarchy, RetrType.Ccomp, ChainApproxMethod.ChainApproxSimple);
            if(this.debug == true)
            {
                Mat copyimg = new Mat();
                frame.CopyTo(copyimg);
                CvInvoke.DrawContours(copyimg, contours, -1, new MCvScalar(0, 255, 0));
                CvInvoke.Imshow("contours image", copyimg);
                CvInvoke.WaitKey();
            }

            VectorOfPoint hulls = new();
            VectorOfVectorOfPoint quads = new();
            for (int i = 0; i < contours.Size; i++) {
                VectorOfPoint contour = contours[i];                                         //取出多边形 get polygon
                Array arr = hierarchy.GetData(true);
                if (contour.Size >= 4 &&  Convert.ToInt32(arr.GetValue(0, i, 1)) < 0){     
                    var area = CvInvoke.ContourArea(contour);                               //求多边形面积 get contour`s area
                    if (area > this.minarea) {
                        VectorOfPoint hull =new();
                        CvInvoke.ConvexHull(contour,hull,false,true);                       //求出凸包 get hull                        
                        if ((area / CvInvoke.ContourArea(hull)) > 0.8){                     
                            hulls.Push(hull);
                            VectorOfPoint quad = new();
                             CvInvoke.ApproxPolyDP(hull, quad, 9, true);                    //根据凸包计算出四边形 get quad
                            if (quad.Size == 4) {                                     
                                var areaqued = CvInvoke.ContourArea(quad);            
                                var areahull = CvInvoke.ContourArea(hull);            
            ...
            if (this.debug == true) {
                Mat copyimg = new ();
                frame.CopyTo(copyimg);
                CvInvoke.DrawContours(copyimg, quads, -1, new MCvScalar(0, 255, 0));
                CvInvoke.Imshow("contours image", copyimg);
                CvInvoke.WaitKey();
                Console.WriteLine("quads count" + quads.Size);
            }

            ArrayList detections = new ();
            ArrayList points = new ();
            ArrayList whitepoints = new ();

            //进行点quad点的提取
            for (var i=0;i<quads.Size;i++)
            {
                VectorOfPoint quad = quads[i];
                int dd = this.tagfamily.getBlackBorder() * 2 + this.tagfamily.getEdge();
                ArrayList blackvalue = new ();
                ArrayList whitevalue = new ();
                ...
                for (int iy = 0; iy < dd; iy++)
                {
                    for (int ix = 0; ix < dd; ix++)
                    {
                        ...
                        var pt = quad.ToArray();
                        var polatepoint = _interpolate(pt, new PointF((float)x, (float)y));
                        points.Add(polatepoint);
                        var grayData = gray.GetData();
                        var value = grayData.GetValue(polatepoint.X, polatepoint.Y);
                ...
                for (int iy = 0; iy < dd; iy++)
                {
                    for (int ix = 0; ix < dd; ix++)
                    {
                        ...
                        Vector2 vector2 = new((float)newx, (float)newy);
                        var pt = quad.ToArray();
                        Point point = _interpolate(pt, new PointF((float)newx, (float)newy));
                        var grayData = gray.GetData();
                        int grayvalue = (byte)grayData.GetValue(point.X, point.Y); 
                        tagcode = tagcode << 1;
            ...
            if (this.debug == true)
            {
                ...
                foreach (Point item in points)
                {
                    ...
                    CvInvoke.Circle(copyimg, tpoint, 1, new MCvScalar(0, 0, 255)); 
                }
                CvInvoke.Imshow("quad", copyimg);
                CvInvoke.WaitKey();
